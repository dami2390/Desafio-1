...
class Contenedor {
  ...
  async _readFileOrCreateNewOne() {
    try {
      ...
      error.code === "ENOENT"
        ...
        : console.log(
            `Error Code: ${error.code} | Hubo un error al leer el archivo ${this._filename}`
          );
    }
  }
  ...
  async _createEmptyFile() {
    fs.writeFile(this._filename, "[]", (error) => {
      error
        ? console.log(error)
        : console.log(`El archivo ${this._filename} fue creado porque no existía`);
    });
  }
  ...
  async deleteAll() {
    try {
      ...
      console.log(
        `Hubo un error (${error.code}) cuando se intentó eliminar todos los objetos`
      );
    }
  }
